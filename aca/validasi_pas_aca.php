...
/** buat no pas sudah kembali pada saat membuat PAS (09/10/2016)
$tahun		= date('y');
$tahun_cek	= date('Y');
$bulan		= date('m');
$urut		= mysql_query ("SELECT max(substring(a.no_pas,11,3)+1) as no FROM `tabel_pas` a, tabel_cmr b, tabel_ao c WHERE a.id_cmr=b.id_cmr and c.id_ao=b.id_ao and YEAR(tgl_pengajuan)='$tahun_cek' 
			and MONTH(tgl_pengajuan)='$bulan' and c.singkatan='$hasil[singkatan]' and a.kategori='PUSAT'");
...
$no_pas = "SML/".$tahun."/".$bulan."/".$no."/".$ao;
*/

$sql = mysql_query ("UPDATE tabel_pas SET `validasi_aca` = 'Valid', tgl_validasi_aca='$tgl' WHERE `tabel_pas`.`id_pas` = '$id_pas'");
	...
	location.href='data_validasi_pas_aca.sml'</script>";
	}else{
	echo "<script>alert('Data gagal disimpan, silakan ulangi proses..')
	location.href='detail_pas_aca.sml?id=$id_pas'</script>";
	}
