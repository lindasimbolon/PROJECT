		...
		<form action=""  method="post" >
			...
			<table width="50%" align="center"><br>  
				...
				<tr align="left" height="80">
					<td></td>
					<td><input type="submit" name="simpan" value="Simpan">&nbsp;&nbsp; <a href="detail_pas_asm.sml?id=<?php echo $id_pas;?>"><img src="images/back.png" width="50px" height="28px"></a></td>
				</tr>
			</table>
		</form>
...
$sql = mysql_query ("UPDATE `sml_pas`.`tabel_pas` SET `persetujuan_asm` = '$persetujuan', `keterangan_asm` = '$keterangan' WHERE `tabel_pas`.`id_pas` = '$id_pas'");
	...
	//proses kirim email ke rpm untuk persetujuan proposal
		...
		$cek_info 	= mysql_query("SELECT * FROM tabel_info where id_info='9'");
	...
	location.href='data_persetujuan_pas_asm.sml'</script>";
	}else{
	echo "<script>alert('Data gagal disimpan, silakan ulangi proses..')
	location.href='persetujuan_pas_asm.php?id=$id_pas'</script>";
	}
}
?>
