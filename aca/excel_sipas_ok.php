...
header("Content-Disposition: attachment; filename=sipas_selesai.xls");
include "../koneksi.php";
include "fungsi_indotgl.php";
?>
		<center><h3>SIPAS SELESAI</h3></center>
		<table border="1" width="100%">
			<tr bgcolor="#2e9fd2" align="center" height="40">
				...
				$sql1 = mysql_query ("SELECT * FROM `tabel_sipas` a INNER JOIN tabel_pas b on a.id_pas=b.id_pas inner join tabel_cmr d on b.id_cmr=d.id_cmr inner join tabel_ao e on d.id_ao=e.id_ao WHERE e.id_ao<>'5' and e.id_ao<>'6' and e.id_ao<>'7' and a.status_sipas = 'Selesai' ");
				} elseif($regional=="RPM EAST") {
				$sql1 = mysql_query ("SELECT * FROM `tabel_sipas` a INNER JOIN tabel_pas b on a.id_pas=b.id_pas inner join tabel_cmr d on b.id_cmr=d.id_cmr inner join tabel_ao e on d.id_ao=e.id_ao WHERE e.id_ao<>1 and e.id_ao<>2 and e.id_ao<>3 and e.id_ao<>4 and a.status_sipas = 'Selesai'");
				}else{
				$sql1 = mysql_query ("SELECT * FROM `tabel_sipas` a INNER JOIN tabel_pas b on a.id_pas=b.id_pas inner join tabel_cmr d on b.id_cmr=d.id_cmr inner join tabel_ao e on d.id_ao=e.id_ao WHERE a.status_sipas = 'Selesai'");
				}
