...
if(isset($_POST['simpan'])){
	if($keterangan<>""){
		...
		$sql = mysql_query ("UPDATE tabel_pdm SET `persetujuan_aca` = '$persetujuan', `komentar_aca` = '$keterangan', tgl_persetujuan_aca='$tgl' WHERE `tabel_pdm`.`id_pdm` = '$id_pdm'");
			...
			//proses kirim email ke rpm untuk persetujuan proposal
				...
				$message	= "Dear Bapak RPM,<br><br><br>
						...
						Kepada Bpk RPM dimohon untuk Persetujuan PDM yang diajukan dari CMR di Area Office yang bapak pimpin.<br><br>
						
						silakan login untuk melakukan Persetujuan PDM: <a href=pas.apachesml.co.id> http://pas.apachesml.co.id</a>
						<br><br>
		...
		$sql 	= mysql_query ("UPDATE tabel_pdm SET `persetujuan_aca` = '$persetujuan', `komentar_aca` = '$keterangan', tgl_persetujuan_aca='$tgl', status_pdm='Ditolak' WHERE `tabel_pdm`.`id_pdm` = '$id_pdm'");
		...
		$sql1	= mysql_query("UPDATE tabel_pas SET sisa_pdm = sisa_pdm + '".$cek['total_pdm']."' WHERE id_pas = '".$cek['id_pas']."'");
			if(($sql)and($sql1)){
			//proses kirim email ke rpm untuk persetujuan proposal
				$cek_regional = mysql_query ("SELECT * FROM tabel_pas a, tabel_cmr b, tabel_pdm c WHERE a.id_pas=c.id_pas and a.id_cmr=b.id_cmr and 
						c.id_pdm='$id_pdm'");
				$hasil_regional = mysql_fetch_array($cek_regional);
