...
if(isset($_POST['simpan'])){
	if($keterangan<>""){
		...
		$sql = mysql_query ("UPDATE tabel_pas SET `persetujuan_rma` = '$persetujuan', `keterangan_rma` = '$keterangan', tgl_persetujuan_rma = '$tgl' WHERE `tabel_pas`.`id_pas` = '$id_pas'");

			if($sql){
			
			
				$message	= 	"Dear Bapak Deputy Regional Manager,<br><br><br>
	
	
							Pengajuan PAS telah di validasi oleh RMA, dimohon untuk Bpk. Deputy Regional Manager untuk melakukan persetujuan<br>
							
							Silakan login untuk melakukan persetujuan, Anda dapat mengakses sistem dengan alamat: <a href=pas.apachesml.co.id> https://pas.apachesml.co.id</a>
							<br><br>
