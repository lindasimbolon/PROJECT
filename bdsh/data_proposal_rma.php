...
<!-- Header -->
<header id="head">
		<h2><b>DATA VERIFIKASI PROPOSAL </b></h2>
<div style="padding:3px;overflow:auto;width:auto;height:550px;font-size:12px;">
		<table id="contoh">
			<thead >  
			...
			<tbody>
				<?php
				$i=0;
				$sql1 = mysql_query ("SELECT * FROM tabel_proposal a, tabel_cmr b, tabel_ao d where a.id_cmr=b.id_cmr and d.id_ao=b.id_ao and (((persetujuan_asm ='Disetujui') and 
				persetujuan_rma is null and ((jenis_proposal='PAS_<25')or (jenis_proposal='PAS_25_50')or (jenis_proposal='PAS_>50')or(jenis_proposal='PUSAT_KDM'))) or(persetujuan_rma is null and(jenis_proposal='LOKAL_DIREKSI')))order by a.id_proposal DESC");
				...
				<tr height="30">
					...
					<td align="left">
					<?php echo substr($hasil1['nama_program'],0,20);?> ...</a>
					</td>
					<td align="center">
					<?php 
					...
					<td align="center"><?php if($hasil1['jenis_proposal']<>"LOKAL_DIREKSI"){if ($hasil1['persetujuan_asm']==""){ echo Pending;}else { echo $hasil1['persetujuan_asm'];}}?></td>
					<td align="center"><?php if ($hasil1['persetujuan_rma']==""){ echo Pending;}else { echo $hasil1['persetujuan_rma'];}?></td>
					<td align="center"><?php if($hasil1['jenis_proposal']<>"LOKAL_DIREKSI"){if ($hasil1['persetujuan_rpm']==""){ echo Pending;}else { echo $hasil1['persetujuan_rpm'];}}?></td>
					<td align="center"><?php if(($hasil1['jenis_proposal']=="PUSAT_KDM")or($hasil1['jenis_proposal']=="LOKAL_DIREKSI")){if ($hasil1['persetujuan_rm']==""){ echo Pending;}else { echo $hasil1['persetujuan_rm'];}}?></td>
					...
					($hasil1['jenis_proposal']=='LOKAL_DIREKSI')){echo "_lokal";}elseif($hasil1['jenis_proposal']=='PUSAT_KDM'){echo "_kdm";}else{echo "_baru";}?>_rma.sml?id=<?php echo $hasil1['id_proposal'];?>"><img src='images/detail.png' width='12px' title='detail'></a>&nbsp;
