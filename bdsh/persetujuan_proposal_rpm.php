...
if(isset($_POST['simpan'])){
	if($keterangan<>""){
		if ($persetujuan=="Disetujui"){
			$sql = mysql_query ("UPDATE tabel_proposal SET `persetujuan_rpm` = '$persetujuan', `keterangan_rpm` = '$keterangan', `tgl_persetujuan_rpm` = '$tgl', `status_pas` = 'Belum PAS' WHERE `tabel_proposal`.`id_proposal` = '$id_proposal'");
	
			if($sql){
			
			//proses kirim email ke rpm untuk persetujuan proposal
				$cek_email 	= mysql_query ("SELECT * FROM tabel_proposal a, tabel_cmr b, tabel_ao c, tabel_territory d where d.nama_spv=a.nama_spv and c.id_ao=b.id_ao and a.id_cmr=b.id_cmr and a.id_proposal='$id_proposal'");
				$hasil_email 	= mysql_fetch_array($cek_email);
				
				if($hasil_email['jenis_proposal']=="PUSAT_KDM"){
					...
					// More headers
					$headers .= 'From: <PAS ONLINE>' . "\r\n";
					$headers .= 'Cc: '.$spv.','.$asm.',marisa@apachesml.co.id' . "\r\n";
					$headers .= 'Bcc: willy@apachesml.co.id,frans@apachesml.co.id,rolina@apachesml.co.id' . "\r\n";
					@mail($to,$subject,$message,$headers);
				}
			...
			//proses kirim email ke rpm untuk persetujuan proposal
				...
				$message	= 	"Dear Bapak CMR,<br><br><br>
							...
							Proposal PAS Anda, Anda dapat mengakses sistem dengan alamat: <a href=pas.apachesml.co.id> http://pas.apachesml.co.id</a>
							<br><br>
				...
				// More headers
				$headers .= 'From: <PAS ONLINE>' . "\r\n";
				$headers .= 'Cc: '.$hasil_email['asm'].',marisa@apachesml.co.id' . "\r\n";
				$headers .= 'Bcc: willy@apachesml.co.id,frans@apachesml.co.id,rolina@apachesml.co.id' . "\r\n";
				@mail($to,$subject,$message,$headers);
